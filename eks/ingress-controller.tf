...
# - Network Load Balancers (NLB) for LoadBalancer Services
#
# Prerequisites:
# - OIDC provider configured (already done in iam.tf)
# - Controller installed via Helm (see manifests/ingress/README.md)
...
# -----------------------------------------------------------------------------
# Allows the controller's ServiceAccount to assume this role
...
resource "aws_iam_role" "alb_controller" {
  name = "${var.project_name}-eks-alb-controller-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:${local.alb_controller_namespace}:${local.alb_controller_service_account}"
